...
int model::save_model(string model_name) {
    ...
    if (save_model_perplexity(dir + perplexity_suffix)) {
    return 1;
    }
...
int model::save_model_perplexity(string filename) {
    ...
    long double perplexity;
    long double sum = 0;
    long double sum_log_p_w_Model=0;
    long num_words=0;
    for (int m = 0; m < M; ++m){
      for(int i=0; i< ptrndata->docs[m]->length; ++i){ //iterate over all the words in doc m
	num_words++;
	int t = ptrndata->docs[m]->words[i]; // t = i_th word in document m
	sum=0;
	for (int k = 0; k < K; ++k) {
	  sum += phi[k][t] * theta[m][k];   
        }
	sum_log_p_w_Model+= log(sum);
      }//i
    }//m


    perplexity = exp ( - sum_log_p_w_Model/static_cast<double>(num_words));

      //    sum /= (V*K);
      //    long double power = exp(sum);
      //    perplexity = 1/power;
